...
// W25QXX 驱动代码
uint16_t W25QXX_TYPE = W25Q128; // 默认是W25Q16
...
void W25QXX_Erase_Chip(void)
{
    ...
    W25QXX_CS_DESELECT;                // 取消片选
    W25QXX_Wait_Long_Busy();           // 等待芯片擦除结束
}
...
void W25QXX_Wait_Long_Busy(void)
{
    while ((W25QXX_ReadSR() & 0x01) == 0x01)
    {
        osDelay(1); // 等待BUSY位清空
...
{
    W25QXX_Read((uint8_t *)info, 0x400, 20);
    if (info[0] != 'h')
    {
        sprintf(info, "hello world\r\n");
        W25QXX_Write((uint8_t *)info, 0x400, strlen(info));
    }
}
