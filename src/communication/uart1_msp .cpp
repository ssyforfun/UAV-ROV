...
UART_HandleTypeDef *huart1Handle = &UartHandle;
void (*uart1_init)(void) = uart_default_init;
#define BAUD_RATE (115200) // 波特率, 常见有: 9600, 115200, 38400
#define USE_UART1 1
#define USART_INSTANCE USART1
#define USART_INSTANCE_CLOCK_ENABLE() __HAL_RCC_USART1_CLK_ENABLE()
...
#define USART_INSTANCE_TX_DMA_STREAM DMA2_Stream7
#define USART_INSTANCE_RX_DMA_STREAM DMA2_Stream5
#pragma endregion
...
static void uart_default_init(void)
{
    ...
    UartHandle.Instance = USART_INSTANCE;                         // USART3  这一行应该是写重复了，只初始化了USART1，问题不大
